                ...
                <form action="{{route('exam.update',$exam->id)}}" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input name="location" type="text" class="form-control" id="exampleInputEmail1"
                               aria-describedby="emailHelp" placeholder="Enter location" value="{{$exam->location}}">
                    ...
                    <div class="form-group">
                        ...
                        <input name="test_hour" type="text" class="form-control" id="test_hour"
                               placeholder="ex : 9 am"
                               value="{{$exam->test_hour}}">
                    </div>
                    <div class="form-group">
                        <label for="type">Exam type </label>
                        <select name="type" class="form-control" id="type">
                            <option value="quiz" @if($exam->type ==="quiz") selected @endif>quiz</option>
                            <option value="midterm" @if($exam->type ==="midterm") selected @endif>midterm</option>
                            <option value="final" @if($exam->final ==="final") selected @endif>final</option>
                        </select>
                    </div>
                    {{csrf_field()}}
                        ...
                        <select name="course_id" class="form-control" id="exampleFormControlSelect2">
                            @foreach($courses as $course)
                                <option value="{{$course->id}}"
                                        @if($exam->course_id == $course->id) selected @endif>{{$course->name.', code: '. $course->code }}</option>
                            @endforeach
                        </select>
